...
class Repository extends EventEmitter {
      ...
      if (stats.isDirectory()) {
        ...
        if (
          ...
          if (files.includes("inventory.json")) {
            ...
            await this.__findObjectsFilesystemRepository({ root: potentialObject });
          }
        }  
    } 
    if(root === this.ocflRoot) {
      this.emit('done');
    }   
  }
  ...
  async __findObjectsS3Repository() {
    ...
    while (walk) {
      ...
      objects.Contents.forEach(async (entry) => {
        if (entry.Key.match(/.*\/0=ocfl_object_1.0/)) {
          let objectPath = path.dirname(entry.Key);
          let inventory = await this.bucket.readJSON({
            target: path.join(objectPath, "inventory.json"),
          });
          let object = this.object({ id: inventory.id });
          this.emit("object", object);
        }
